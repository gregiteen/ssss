...
| `../registry/core.json` | The 13 core document primitives + 5 contract primitives, plus the `bundle` (§16) and `provisioning` (§17) schemas the standard owns. |

## Implementations under test

| Implementation | SSSS surface | Core primitives | Extensions |
|----------------|--------------|-----------------|------------|
| **festech.live** | `SsssOperationService` + `SsssLeaseService`, Postgres event log, ~4,024 vault files | all 13 ✓ | 118 app primitives |
...
and round-trips the reference bundle (validate → provision → import → re-import,
spec §16/§17) against a temp vault — proving idempotency and that no `tenant_private`
file ever lands. With `--endpoint`, it POSTs each fixture's `request` to a host's
Operation Contract endpoint and diffs the response.

## Core vs. extension registry

The standard owns only the **core**: the 13 document primitives and 5 contract
primitives in [`../registry/core.json`](../registry/core.json), plus the `bundle`
(§16) and `provisioning` (§17) schemas. Every conformant implementation maps to the
same core.
...
| 2 | **Error codes** | §6.5 lists `401, 403, 409, 422, 500`. | Also returns `400` (invalid request / path traversal) and `404` (patch/delete target missing). | **Amend §6.5** to add `400` and `404`. |
