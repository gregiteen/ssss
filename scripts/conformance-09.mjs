...
import { ProjectionCoordinator } from '../src/projections.mjs';
...
export async function runKernel09Conformance() {
  ...
  try {
    ...
    const kernel = createKernel({ vfs, eventStore, projectionCoordinator: coordinator, validator });
    const principal = {
      id: 'greg', kind: 'human', workspaceIds: ['acme'], capabilities: ['booking:create', 'booking:update', 'booking:delete'],
    ...
    const patched = await kernel.execute({ type: 'patch', workspace_id: 'acme', idempotency_key: 'patch-booking', path: 'bookings/one.md', patches: { status: 'confirmed' } }, { principal });
    check('shared kernel patches and events the primitive', patched.success && await eventStore.size() === 2);
    ...
    check('projection is rebuildable from the event log', rebuilt.applied === 2 && projected.has('bookings/one.md'));
